...
# from .database import add_song_metadata
...
def parse_recommendations(recommendations: List[dict]):
...
def recommendations_by_artist(seed_artists_ids: list, number_of_recs: int = 10):
    ...
    Returns:
        dict: A list of songs
    """
    if isinstance(seed_artists_ids, str):
        recommanded_songs = spotify.recommendations(seed_artists=[seed_artists_ids], limit=number_of_recs)
        return recommanded_songs
    recommanded_songs = spotify.recommendations(seed_artists=seed_artists_ids, limit=number_of_recs)
    return parse_recommendations(recommanded_songs)


def recommendations_by_genre(seed_genre: str, number_of_recs: int = 10):
    """Get song recommendations based on an artists. If multiple artists are passed, recommendations will
    be based on the various artists
